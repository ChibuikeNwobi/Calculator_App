...
export default function MyKeyboard() {
  ...
  const getResult = () => {
    switch (operation) {
      case "+":
        clear();
        setResult(parseInt(secondNumber, 10) + parseInt(firstNumber, 10));
        break;
      case "-":
        clear();
        setResult(parseInt(secondNumber, 10) - parseInt(firstNumber, 10));
    ...
    if (result !== null) {
      return (
        <Text
          style={
            result < 99999
              ? [Styles.screenFirstNumber, { color: myColors.results }]
              : [Styles.screenFirstNumber, { fontSize: 50, color: myColors.results }]
          }
        >
          {result.toString()}
        </Text>
